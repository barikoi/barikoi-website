...
class Slider extends Component {
  ...
  handleDrag = (e) => {
    ...
    // Code added to do collisions in pricing api
    let tooltip = document.getElementById('tooltip');
    let minRequest = document.getElementById('min-request');
    let maxRequest = document.getElementById('max-request');

    //console.log('tootltip' + tooltip);

    if (minRequest) {
      this.isCollapsed(tooltip, minRequest);
    }

    if (maxRequest) {
      this.isCollapsed(tooltip, maxRequest);
    }
  };
  ...
  isCollapsed = (tooltip, request) => {
    if (tooltip !== null && request !== null) {
      ...
      var object_2 = request.getBoundingClientRect();
      ...
      if (
        ...
        request.classList.add('collide');
      } else {
        request.classList.remove('collide');
      }
    }
  };
